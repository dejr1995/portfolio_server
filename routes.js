...
routes.get('/col_left/:id', (req, res) => {
    req.getConnection((err, conn) => {
        ...
        const { id } = req.params;
        conn.query('SELECT * FROM column_left WHERE id = ?', [id], (err, result) => {
            if (err) {
...
routes.post('/col_left', (req, res) => {
    req.getConnection((err, conn)=>{
    const { id, urlvideo, title, skill1, skill2, skill3, skill4, skill5, urlopen, urlgithub, urlprototype, type } = req.body;
    conn.query(
        'INSERT INTO column_left (id, urlvideo, title, skill1, skill2, skill3, skill4, skill5, urlopen, urlgithub, urlprototype, type) VALUES (?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?)',
        [id, urlvideo, title, skill1, skill2, skill3, skill4, skill5, urlopen, urlgithub, urlprototype, type],
        (err, result) => {
            if (err) {
...
routes.put('/col_left/:id', (req, res)=>{
    req.getConnection((err, conn)=>{
        if(err) return res.send(err)
        conn.query('UPDATE column_left set ? WHERE id = ?', [req.body, req.params.id], (err, result)=>{
            if (err) {
...
routes.get('/col_center/:id', (req, res) => {
    req.getConnection((err, conn) => {
        ...
        const { id } = req.params;
        conn.query('SELECT * FROM column_center WHERE id = ?', [id], (err, result) => {
            if (err) {
                console.error('Error al obtener persona por ID: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
...
routes.delete('/col_center/:id', (req, res)=>{
    req.getConnection((err, conn)=>{
        const { id } = req.params;
        if(err) return res.send(err)
        conn.query('DELETE FROM column_center WHERE id = ?', [id], (err, result)=>{
            if (err) {
                console.error('Error al eliminar persona: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
...
routes.put('/col_center/:id', (req, res)=>{
    req.getConnection((err, conn)=>{
        if(err) return res.send(err)
        conn.query('UPDATE column_center set ? WHERE id = ?', [req.body, req.params.id], (err, result)=>{
            if (err) {
                console.error('Error al actualizar persona: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
...
routes.get('/col_right/:id', (req, res) => {
    req.getConnection((err, conn) => {
        ...
        const { id } = req.params;
        conn.query('SELECT * FROM column_right WHERE id = ?', [id], (err, result) => {
            if (err) {
                console.error('Error al obtener persona por ID: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
...
routes.put('/col_right/:id', (req, res)=>{
    req.getConnection((err, conn)=>{
        if(err) return res.send(err)
        conn.query('UPDATE column_right set ? WHERE id = ?', [req.body, req.params.id], (err, result)=>{
            if (err) {
                console.error('Error al actualizar persona: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
...
routes.get('/image/rand', (req, res)=>{
    req.getConnection((err, conn)=>{
        ...
        conn.query('SELECT * FROM gallery ORDER BY RAND() LIMIT 5', (err, result)=>{
            if (err) {
                console.error('Error al obtener personas: ' + err.message);
                res.status(500).json({ error: 'Error en el servidor' });
            } else {
